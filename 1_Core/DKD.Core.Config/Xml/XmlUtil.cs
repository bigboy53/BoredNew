...
namespace DKD.Core.Config.Xml
{
    ...
    public class XmlUtil
    {
        ...
        /// </summary>
        /// <param name="source">XML资源，文件为路径，否则为XML字符串</param>
        /// <param name="tableName">解析节点名称</param>
        /// <returns>DataTable</returns>
        ...
        public static Dictionary<string, object> GetDictionary(string source, string fisertnodes)
        {
            string fileName = source;
            XmlDocument doc = new XmlDocument();
            doc.Load(fileName);
                ...
                for (int i = 0; i < songNode.ChildNodes.Count; i++)
                {
                    if (songNode.NodeType != XmlNodeType.Comment)
                    {
                        ...
                        if(colName != "#comment")
                        {
                            ...
                            {
                                dic.Add(colName,songNode.ChildNodes.Item(i).InnerText.Trim());
                            }
                        }
                    }
                }
