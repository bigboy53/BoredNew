...
namespace DKD.Framework.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        private static readonly Common.IPAddress IpSearch = __getIPConfig();

        private static Common.IPAddress __getIPConfig()
        {
            try
            {
                if (HttpContext.Current != null)
                    return new Common.IPAddress(HttpContext.Current.Server.MapPath(Config.ConfigBase.Instance<Config.FrameworkConfig>().IPDatabasePath));
                return new Common.IPAddress(System.IO.Path.Combine((AppDomain.CurrentDomain.RelativeSearchPath ?? AppDomain.CurrentDomain.BaseDirectory), Config.ConfigBase.Instance<Config.FrameworkConfig>().IPDatabasePath));
            }
            catch { return null; }
        }
